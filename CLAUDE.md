# EgorRobot — Project Context for AI Assistants

**Read this first.** It captures everything a new Claude/AI session (or another
chatbot) needs to work on this robot without re-deriving the hard-won findings.
...
- **Resilience:** CAN send errors (e.g. a dropped wire → ENOBUFS / no-ACK) are
  ...
  in `/api/status` instead of crashing (which would kill the watchdog).
...
- **Shutdown:** `POST /api/shutdown` stops the motors then runs `sudo shutdown -h
  now` to power the Pi off cleanly. The server runs as a non-root user, so
  `utils/setup_shutdown_api.sh` installs a narrow password-less-sudo rule for only
  `shutdown`; without it the endpoint returns a permission error. The page's ⏻
  button confirms first. (The GPIO shutdown button runs the same command directly;
  there is no separate reusable shutdown service.)

Endpoints: `POST /api/move/<DIR>`, `POST /api/drive {left,right}`, `POST /api/stop`,
`POST /api/estop`, `POST /api/enable`, `POST /api/shutdown`, `POST /api/reboot`,
`GET /api/status`, `GET /api/netinfo`, `GET /api/sysinfo`, `GET /` (UI),
`GET /admin` (admin page), `GET /api`.
DIR ∈ FWD, REV, FWD_L, FWD_R, REV_L, REV_R, SPIN_L, SPIN_R, STOP. Turn arcs use an
inner-wheel fraction of 0.3 (tunable in `MOVES`); base speed 4.0 rad/s.
...
page) is a live health panel refreshed every 3 s, backed by two read endpoints:
...
- Service `egor-camera.service` (installer `utils/setup_camera_service.sh`) runs
  `ustreamer -d /dev/video0 -r 640x480 -m MJPEG -f 15 -s 0.0.0.0 -p 8081`. Stream
  at `http://<host>:8081/stream`, viewer page at `http://<host>:8081/`.
- `robot_ui.html` embeds the stream as an `<img>` whose URL is built from the
  ...
  "camera offline" overlay. A cross-origin `<img>` needs no CORS.
...
- Tunables: resolution/fps at the top of `setup_camera_service.sh`; video-panel
  `max-width` (`.cam`) and button sizes in `robot_ui.html`.
...
- **`robot_ui.html`** — the joystick web page, served at `/`; also embeds the
  webcam video panel (µStreamer stream on :8081).
